...
void Decoding::decodeBinaryString(const std::string& binaryStr) {
    ...
    // Find the position of ten consecutive '1's
    size_t posTenOnes = binaryStr.find("1111111111");
    // If found, adjust the string to process only up to the ten '1's
    std::string processedStr = (posTenOnes != std::string::npos) ? binaryStr.substr(0, posTenOnes + 10) : binaryStr;

    int currentIndex = 0;

    for (const auto& [codeName, length] : messageCodes) {
        // Ensure that the substring does not exceed the string's length
        if (currentIndex + length > processedStr.size()) {
            break; // Exit the loop if the remaining string is shorter than expected
        }

        // Extract the binary subsequence
        std::string subBinary = processedStr.substr(currentIndex, length);
